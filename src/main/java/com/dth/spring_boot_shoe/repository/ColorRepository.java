...
public interface ColorRepository extends JpaRepository<ColorEntity,Long> {
    ...
    @Query("select p.color from ProductDetailEntity p where p.product.brand.slug=:brandSlug group by p.color.id")
    List<ColorEntity> findColorByProduct(@Param("brandSlug") String brandSlug);
    ...
    @Query("select c from ColorEntity c " +
            ...
            "where p.product.id=:productId and p.status <> -1 " +
            "group by p.color.id)")
    ...
    @Query("select p.color from ProductDetailEntity p " +
            ...
            "where p.product.id=:productId and p.status <> -1 " +
            "group by p.color.id) or p.id=:id " +
            "group by p.color.id")
